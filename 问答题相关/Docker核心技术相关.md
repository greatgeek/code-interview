...
2. **memory 控制器**:
   - 当进程尝试分配内存时，`cgroup` 会检查其所在的控制组是否超出了其限制。如果超出了，内存分配可能会失败，或者系统可能会触发 OOM 杀手。
...
这些控制器的工作方式是：当进程发出系统调用请求资源或执行某些操作时，`cgroup`子系统会介入并检查请求与该进程所在`cgroup`的配置之间是否存在冲突。例如，当进程请求内存分配时，`memory` 控制器会检查是否已经达到或超出 `memory.limit_in_bytes` 的值。
